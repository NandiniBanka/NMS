...
$statement = "SELECT 
    ...
    FROM devices where employee_id = '$user_id'";
...
$statements = "SELECT 
    AVG(CASE WHEN dr.id LIKE 'SV%' THEN dr.avg_bw_used ELSE NULL END) AS server_bw,
    AVG(CASE WHEN dr.id LIKE 'SW%' THEN dr.avg_bw_used ELSE NULL END) AS switch_bw,
    AVG(CASE WHEN dr.id LIKE 'FW%' THEN dr.avg_bw_used ELSE NULL END) AS firewall_bw,
    AVG(CASE WHEN dr.id LIKE 'RT%' THEN dr.avg_bw_used ELSE NULL END) AS router_bw
    FROM device_report dr
    JOIN devices d ON dr.id = d.id
